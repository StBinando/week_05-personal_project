...
def select_filtered(filter = "all"):
    ...
    # IT HAS TO GO LOOK INTO ANOTHER TABLE
    if filter == "pre_booked":
        results = customer_item_repository.show_all()

        # CREATES A LIST OF UNIQUE VALUES FOR ITEMS IDs
        # THAT HAVE BEEN Pre_ORDERED BY CUSTOMERS
        item_ids = []
        for row in results:
            item_ids.append(row.item.id)
        item_ids = set(item_ids)

        # BASED ON THAT LIST, IT CREATES A LIST OF
        # ITEM OBJECTS THAT MATCH THE IDs FOUND
        items = []
        for id in item_ids:
    ...
    # OTHERWISE...
        ...
        # IT SIMPLY SETS SQL TO SELECT EVERYTHING 
        if filter == "all":
            sql = "SELECT * FROM items"

        # AND IF THE FILTER IS ONE OF THE OTHER TWO
        # IT SETS THE SQL TO RETRIEVE ALL ITEMS WHERE 
        # THE COLUMN MATCHING THE FILTER NAME IS > 0
    ...
    # AND FINALLY RETURNS WHATEVER LIST IT HAS PRODUCED
    return items
